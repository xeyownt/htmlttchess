...
var MERIDA_FRAME_ROW_PADDING =
    MFB + '<br/>' + MFB + '<br/>' + MFB + '<br/>' + MFB + '<br/>' + MFB + '<br/>' + MFB + '<br/>' + MFB + '<br/>' + MFB + '<br/>';
            ...
            if (attr == VALUE_BORDER_SOLID) {
                ...
                for (x = 0; x < attr.length; x++) {
                    switch (attr.charAt(x)) {
                        case 'n':
                            this.isCoordShownTop = true;
                            break;
                        case 's':
                            this.isCoordShownBottom = true;
                            break;
                        case 'o':
                            this.isCoordShownLeft = true;
                            break;
                        case 'e':
                            this.isCoordShownRight = true;
                            break;
                    }
                }
                if (!isCoordShown) {
    ...
    this.genColorHTML = function () {
        ...
        if (this.isCoordShownTop) {
            ...
            generatedHTML +=
                (this.isCoordShownLeft || this.isCoordPadded ? '<td>&nbsp;</td>' : '') +
                '<td class="cols">' +
                MERIDA_FRAME_COL_PADDING.substr(0, this.colCount) +
                '</td>' +
                (this.isCoordShownRight || this.isCoordPadded ? '<td>&nbsp;</td>' : '') +
                '';
            generatedHTML += '</tr>';
        ...
        for (row = 0; row < this.rowCount; row++) {
            ...
            for (col = 0; col < inputRow.length; col++) {
                ...
                if (cell == '(') {
                    if (!isHLOn) {
                        outputSqRow += '<span class="' + VALUE_CLASS_COLOR_SPAN_HL + '">';
                    }
                    isHLOn = true;
                } else if (cell == ')') {
                    if (isHLOn) {
        ...
        var boardStyle =
            (this.isBorderShownTop ? '' : 'border-top:none !important;') +
    ...
    this.genBWHTML = function () {
        ...
        if (this.isBorderStyleSimple) {
            boardTop = this.isCoordShownTop
                ? this.isBorderShownTop
                    ? MERIDA_FRAME_S_TOP_COORD
                    : MERIDA_FRAME_COLS
                : this.isBorderShownTop
                  ? MERIDA_FRAME_S_TOP
                  : MERIDA_FRAME_PADDING;
            boardRight = this.isCoordShownRight
                ? this.isBorderShownRight
                    ? MERIDA_FRAME_S_RIGHT_COORD
                    : MERIDA_FRAME_ROWS
                : this.isBorderShownRight
                  ? MERIDA_FRAME_S_RIGHT
                  : MERIDA_FRAME_PADDING;
            boardBottom = this.isCoordShownBottom
                ? this.isBorderShownBottom
                    ? MERIDA_FRAME_S_BOTTOM_COORD
                    : MERIDA_FRAME_COLS
                : this.isBorderShownBottom
                  ? MERIDA_FRAME_S_BOTTOM
                  : MERIDA_FRAME_PADDING;
            boardLeft = this.isCoordShownLeft
                ? this.isBorderShownLeft
                    ? MERIDA_FRAME_S_LEFT_COORD
                    : MERIDA_FRAME_ROWS
                : this.isBorderShownLeft
                  ? MERIDA_FRAME_S_LEFT
                  : MERIDA_FRAME_PADDING;
            ...
            boardRight = this.isCoordShownRight
                ? this.isBorderShownRight
                    ? MERIDA_FRAME_D_RIGHT_COORD
                    : MERIDA_FRAME_ROWS
                : this.isBorderShownRight
                  ? MERIDA_FRAME_D_RIGHT
                  : MERIDA_FRAME_PADDING;
            boardBottom = this.isCoordShownBottom
                ? this.isBorderShownBottom
                    ? MERIDA_FRAME_D_BOTTOM_COORD
                    : MERIDA_FRAME_COLS
                : this.isBorderShownBottom
                  ? MERIDA_FRAME_D_BOTTOM
                  : MERIDA_FRAME_PADDING;
            boardLeft = this.isCoordShownLeft
                ? this.isBorderShownLeft
                    ? MERIDA_FRAME_D_LEFT_COORD
                    : MERIDA_FRAME_ROWS
                : this.isBorderShownLeft
                  ? MERIDA_FRAME_D_LEFT
                  : MERIDA_FRAME_PADDING;
        ...
        var generatedHTML = '';
        var divWidth =
            (this.isCoordShownLeft || this.isBorderShownLeft || this.isCoordPadded ? 1 : 0) +
            this.colCount +
        ...
        if (this.isCoordShownTop || this.isBorderShownTop || this.isCoordPadded) {
            generatedHTML +=
                (this.isBorderShownLeft && this.isBorderShownTop
                    ? boardCorner.charAt(0)
                    : this.isBorderShownLeft || this.isCoordShownLeft || this.isCoordPadded
                      ? MERIDA_FRAME_BLANK
                      : '') +
                boardTop.substr(this.coordOriginCol, this.colCount) +
                (this.isBorderShownRight && this.isBorderShownTop
                    ? boardCorner.charAt(1)
                    : this.isBorderShownRight || this.isCoordShownRight || this.isCoordPadded
                      ? MERIDA_FRAME_BLANK
                      : '') +
                '<br/>';
        }
        ...
        for (row = 0; row < this.rowCount; row++) {
            ...
            for (col = 0; col < inputRow.length; col++) {
                ...
                if (cell == '(') {
                    if (!isHLOn) {
                        outputRow += '<span class="' + VALUE_CLASS_BW_SPAN_HL + '">';
                    }
                    isHLOn = true;
                } else if (cell == ')') {
                    if (isHLOn) {
            ...
            generatedHTML +=
                (this.isCoordShownLeft || this.isBorderShownLeft || this.isCoordPadded
                    ? boardLeft.charAt(this.coordOriginRow + this.rowCount - 1 - row)
                    : '') +
                outputRow +
                (isHLOn ? '</span>' : '') +
                (this.isCoordShownRight || this.isBorderShownRight || this.isCoordPadded
                    ? boardRight.charAt(this.coordOriginRow + this.rowCount - 1 - row)
                    : '') +
                '<br/>';
        }
        ...
        if (this.isCoordShownBottom || this.isBorderShownBottom || this.isCoordPadded) {
            generatedHTML +=
                (this.isBorderShownLeft && this.isBorderShownBottom
...
function parseChess() {
    ...
    for (idx = 0; idx < chessElt.length; idx++) {
        ...
        var attrMode =
            chessElt[idx].getAttribute(ATTR_MODE) != null
                ? chessElt[idx].getAttribute(ATTR_MODE) == VALUE_MODE_COLOR
                : contextColorMode;
